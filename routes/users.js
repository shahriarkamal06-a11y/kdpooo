...
router.put('/:id', auth, requirePermission('users.update'), async (req, res) => {
  try {
    ...
    const shouldUnsetBatch = oldUser.role === 'student'
      && Object.prototype.hasOwnProperty.call(req.body, 'batch')
      && !req.body.batch;

    if (oldUser.role === 'student') {
      const Batch = require('../models/Batch');
      const hasBatchField = Object.prototype.hasOwnProperty.call(req.body, 'batch');
      const nextBatch = hasBatchField ? (updates.batch || null) : oldUser.batch?.toString();
      
      // Remove from old batch if exists
      if (oldUser.batch && oldUser.batch.toString() !== nextBatch) {
        await Batch.findByIdAndUpdate(
          oldUser.batch,
      ...
      if (nextBatch && nextBatch !== oldUser.batch?.toString()) {
        await Batch.findByIdAndUpdate(
          nextBatch,
      ...
      if (hasBatchField && !nextBatch) {
        delete updates.batch;
      }
    }

    const updateOperation = shouldUnsetBatch
      ? { $set: updates, $unset: { batch: 1 } }
      : { $set: updates };
