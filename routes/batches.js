...
router.get('/', auth, requirePermission('batches.read'), async (req, res) => {
  try {
    ...
    const batchesWithBengaliDays = batches.map(batch => {
      const batchObj = batch.toObject();
      if (batchObj.schedule && batchObj.schedule.days) {
        batchObj.schedule.days = convertEnglishToBengali(batchObj.schedule.days);
      }
      return batchObj;
    });

    res.json({ success: true, batches: batchesWithBengaliDays });
  } catch (error) {
    console.error(error);
...
router.post('/', auth, requirePermission('batches.create'), async (req, res) => {
  try {
    const batchData = { ...req.body };
    
    // Convert Bengali days to English for database storage
    if (batchData.schedule && batchData.schedule.days) {
      batchData.schedule.days = convertBengaliToEnglish(batchData.schedule.days);
    }
    ...
    // Convert back to Bengali for response
    const batchObj = batch.toObject();
    if (batchObj.schedule && batchObj.schedule.days) {
      batchObj.schedule.days = convertEnglishToBengali(batchObj.schedule.days);
    }

    res.status(201).json(batchObj);
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: 'Server error' });
  }
...
// @desc    Update batch
// @access  Private (Admin, Staff)
router.put('/:id', auth, requirePermission('batches.update'), async (req, res) => {
  try {
    const updateData = { ...req.body };
    
    // Convert Bengali days to English for database storage
    if (updateData.schedule && updateData.schedule.days) {
      updateData.schedule.days = convertBengaliToEnglish(updateData.schedule.days);
    }
...
router.delete('/:id', auth, requirePermission('batches.delete'), async (req, res) => {
  try {
...
// @access  Private (Admin, Staff)
router.put('/:id/students', auth, requirePermission('batches.update'), async (req, res) => {
  try {
...
// @access  Private (Admin, Staff)
router.delete('/:id/students/:studentId', auth, requirePermission('batches.update'), async (req, res) => {
  try {
    const batch = await Batch.findById(req.params.id);
    if (!batch) {
      return res.status(404).json({ message: 'Batch not found' });
